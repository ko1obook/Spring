...
public class DefaultSalesPeriodService implements SalesPeriodService {
    ...
    public SalesPeriod update(SalesPeriod salesPeriod) {
        ...
        List<SalesPeriod> openSalesPeriods = salesPeriodRepository.findByDateToIsNullAndProductId(existingSalesPeriod.getProduct().getId());
        if (!openSalesPeriods.isEmpty()) {
            throw new EntityConflictException(String.format(
                    "Нельзя открыть торговый период с продуктом с id %d, так как у него уже есть открытые торговые периоды", existingSalesPeriod.getProduct().getId()));
        }
        ...
        // Обновляем DateTo, если дата не передана - торговый период открывается
